...
No build step — Flask serves directly. Production: `gunicorn app:app --bind 0.0.0.0:$PORT` (see `railway.toml`).
...
- `database/db.py` — connection helper (`get_db()`), schema creation (`init_db()`), seed data (`seed_db()`). Write operations: `create_user`, `create_expense`, `update_expense`, `delete_expense`. Read operations: `get_user_by_email`, `get_user_by_id` (returns raw `sqlite3.Row`), `get_expense_by_id`. All functions open a connection, use `try/finally` to close it, and use parameterised queries.
- `database/queries.py` — read-heavy queries used by the profile page: `get_user_by_id` (returns a formatted `dict` with `member_since`, **not** the same as `db.get_user_by_id`), `get_summary_stats`, `get_recent_transactions`, `get_category_breakdown`. All four accept optional `date_from`/`date_to` parameters handled by the `_date_filter` helper.

**App constants (app.py):**
- `EXPENSE_CATEGORIES` — canonical list of valid categories; validated server-side on expense submission.
- `MAX_AMOUNT` — upper bound for expense amounts (10,000,000).

**Form validation:** `_validate_expense_form(form)` in `app.py` is shared by both `add_expense` and `edit_expense`. Returns `(amount, category, date_str, description, error)`.

**Session:** Flask session stores `user_id` (int) and `user_name` (str) after login/register. Auth guard pattern: `if not session.get("user_id"): return redirect(url_for("login"))`. Ownership violations (wrong user's expense) redirect to `/profile`, not a 403.
...
- Step 08 — Edit expense (✓ complete)
- Step 09 — Delete expense (✓ complete)
...
Tests live in `tests/` and use **two different strategies**:

- **Older tests** (`test_backend_connection.py`, `test_06_date_filter_profile_page.py`) run against the real seeded SQLite database with no conftest.py and no mocking. They assume the seed user (`id=1`, `demo@spendly.com`) and its 8 seed expenses are present.
- **Newer tests** (`test_07-add-expenses.py`, `test_08-edit-expenses.py`) use a `tmp_path` fixture that monkey-patches `database.db.DB_PATH` to an isolated on-disk SQLite file per test, giving a clean slate each run.

If `expense_tracker.db` is missing or empty, `init_db()` + `seed_db()` run automatically when `app.py` is imported.
